...
- Mention asks are queued per user, so one user’s repeated questions run sequentially without blocking other users.
...
- Mention questions, for example `@bot what is DSSI?`.
  Mention request queue/progress updates are sent privately to the requesting user. For normal message mentions, the bot uses DMs because ephemeral replies are not available.
- `/ping`.
